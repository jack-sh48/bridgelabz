module AccountBlock
  class AccountsController < ApplicationController
    ...
    before_action :validate_json_web_token, only: [:update, :destroy]
    ...
    def destroy
      ...
      if @account.present? && @account.destroy
        render json: { meta: { message: 'account deleted successfully' } }, status: :ok
      else
        render json: { meta: { message: 'account not found.' } }, status: :not_found
      end     
    end
    ...
    def index
      ...
      if @accounts.present?
        ...
        render json: {errors: "accounts data does not exist"}, status: :not_found
      end
    end
    ...
    def update
      ...
      if @account.present? && @account.update(set_account_params)
    ...
    def set_account_params
      params.require(:account).permit(:first_name, :last_name, :email, :password, :full_phone_number)
    end
